...
Before emitting status, ensure `completedPhases` exists in config.json (initialize as `[]` if undefined), then append `'phase-4'` to `completedPhases` in `{systemPath}/.design-farmer/config.json`. If `'phase-4'` is already present, skip the append (idempotent). Also update `config.backup.json`.
