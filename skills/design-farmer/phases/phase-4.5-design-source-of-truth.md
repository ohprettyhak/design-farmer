...
- `{date}` — Initial DESIGN.md generation from Phases 1–4b outputs
- `{date}` — Updated tokens, typography, or component guidance after a new design decision
- `{date}` — Adjusted theme strategy, styling approach, or layout guidance during iteration

Use short, decision-focused entries. Capture meaningful design-direction changes, not every minor edit.
...
If a **draft DESIGN.md** already exists (generated by Phase 3 for context resilience):
1. Read the existing draft — it contains the `## Config` block, color palette, typography, and spacing from Phase 3.
2. **Replace** the draft header (`> **DRAFT**`) with the final header (`> Auto-generated by Design Farmer.`).
3. **Preserve** the extraction data from Phase 3 (colors, typography, spacing) — merge with Phase 4/4b decisions rather than regenerating from scratch. **Conflict resolution:** if Phase 3 draft data conflicts with Phase 4/4b decisions (e.g., dark mode color values adjusted during preview approval), read the user-adjusted values from `DesignFarmerConfig` (which stores the latest approved state) and use those. Priority order: (1) `DesignFarmerConfig` fields (latest approved), (2) Phase 4/4b decisions (user-confirmed during preview gate), (3) Phase 3 draft data (initial extraction).
4. **Add** the sections that Phase 3 could not populate: Visual Theme & Atmosphere (§1), Component Stylings (§4), Depth & Elevation (§6), Do's and Don'ts (§7), Responsive Behavior (§8), Decisions Log (§9), and Agent Prompt Guide (§10).

If no draft exists, generate from scratch as described below.

For greenfield projects (`designMaturity = GREENFIELD`) where many placeholder values are uncertain,
use the following greenfield defaults:

**Greenfield defaults** (if `examples/DESIGN.md` is missing or `designMaturity = 'greenfield'`):
- Primary: `oklch(0.55 0.22 250)` (indigo)
- Primary foreground: `oklch(0.98 0.00 250)`
- Surface default: `oklch(0.98 0.00 0)` (near-white)
- Surface elevated: `oklch(1.00 0.00 0)` (white)
- Text primary: `oklch(0.15 0.01 250)` (near-black)
- Text secondary: `oklch(0.45 0.01 250)`
- Border default: `oklch(0.88 0.01 250)`
- Border strong: `oklch(0.72 0.01 250)`
...
2. Append new entries to the `## 9. Decisions Log` section
3. Update sections that have changed (color, typography, spacing)
4. Preserve the history — never delete past decisions
5. Keep the Decisions Log current so document changes remain understandable without a separate manual revision table
