...
- **Sizing rules:** {reference to best-practices.md or custom rules}
...
After the design system is built, tested, documented, and integrated, ship it for review and deployment.

### 11.1 Pre-Ship Verification
...
if [ "$CURRENT_BRANCH" = "main" ] || [ "$CURRENT_BRANCH" = "master" ]; then
  BRANCH_NAME="feat/design-system-$(date +%Y%m%d)"
  git checkout -b "$BRANCH_NAME"
else
  BRANCH_NAME="$CURRENT_BRANCH"
fi

# Stage all generated design system files (be specific)
git add {systemPath}/

# If new files exist outside systemPath (e.g., DESIGN.md, app integration changes)
git add DESIGN.md 2>/dev/null

# Verify clean staging — no secrets, no node_modules, no build artifacts
git diff --cached --stat

# Commit with descriptive message
git commit -m "feat: add design system with {component_count} components

Generated by Design Farmer:
- {token_count} design tokens (primitive + semantic + component)
- {component_count} accessible components
- OKLCH color system with light/dark themes
- Storybook integration ({story_count} stories)
- Full test coverage ({test_count} tests)

Token hierarchy: primitive -> semantic -> component
Color space: OKLCH with APCA contrast validation
