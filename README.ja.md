...
[English](README.md) | [한국어](README.ko.md) | **日本語** | [简体中文](README.zh.md) | [繁體中文](README.zh-TW.md)
...
Design Farmerはこの工程をまるごと自動化します。コードベースを読み取り、すでにあるものを把握したうえで、プロダクション品質のデザインシステムを構築（またはアップグレード）します。トークンファイルの手書きも、カラーパレットのコピペも、当てずっぽうも不要です。
...
フルパイプラインの構成：プリフライト検出、ディスカバリーインタビュー、リポジトリ分析、OKLCHベースのパターン抽出、ビジュアルプレビュー、アーキテクチャ設計、テーマシステム、DESIGN.md生成、トークン実装、コンポーネントライブラリ、Storybook連携、複数レビューア検証、ライブビジュアルQA、ドキュメント整備、アプリ統合、リリース準備。
...
<img src="assets/storybook-components.png" alt="Design Farmerが生成したコンポーネントギャラリー" width="100%" />

上のスクリーンショットはゼロからの新規プロジェクト（グリーンフィールド）で生成した結果です。トークンもコンポーネントもデザイン方針もない状態から作成しています。すでに一部の実装がある（コンポーネント、カラー変数、スタイルガイドなど）リポジトリでは、それを起点にさらに精度の高い結果を生み出します。

> [!TIP]
> **より良い結果を得るには？** プロジェクトルートに [`DESIGN.md`](https://github.com/VoltAgent/awesome-design-md) を配置してから実行してください。
> - [Stitch](https://stitch.withgoogle.com)で生成する、または
> - [awesome-design-md](https://github.com/VoltAgent/awesome-design-md)から取得する。Vercel、Linear、Stripeなど実在するサイトから抽出した58以上のデザインシステムが揃っています。

## インストール

### Claude Code — マーケットプレイス（推奨）

Claude Code マーケットプレイスから直接インストールすると、プラグインのライフサイクルを Claude Code が管理します：

1. Claude Code の設定を開き、**Plugins → Marketplace** へ移動します。
2. **design-farmer** を検索し、**Install** をクリックします。

Claude Code のコマンドインターフェースを使いたい場合は、このリポジトリをマーケットプレイスとして登録してからプラグインを直接インストールできます：
...
`marketplace add` はマーケットプレイスのカタログ登録です。その後の `install` で登録済みマーケットプレイスから `design-farmer` プラグインを取得し、以後の更新は Claude Code の通常のプラグイン更新フローで管理されます。
...
**Claude Code**、**Codex CLI**、**Amp**、**Gemini CLI**、**OpenCode** を自動検出してインストールします。

マーケットプレイス UI/CLI のインストール手順、マーケットプレイスと curl インストール間の移行、ターゲット選択フラグ（`--tool`、`--interactive`、`--dry-run`）、手動インストール、トラブルシューティング、アンインストール手順は [INSTALLATION.md](INSTALLATION.md) を参照してください。

## ドキュメント

- [インストールガイド](INSTALLATION.md)：マーケットプレイス／curl インストール、手動セットアップ、トラブルシューティング、アンインストールまでカバーする公式リファレンス
- [スキル仕様](skills/design-farmer/SKILL.md)：実行時に参照される指示ファイル

メンテナー・コントリビューター向け：

- [内部プロジェクト契約](docs/README.md)：リポジトリ実装契約と計画記録
