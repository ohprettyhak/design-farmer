...
| Risk | Mitigation |
|------|------------|
| Claude Code plugin schema gains new required fields that the sync script does not propagate. | Release script re-runs `claude plugin validate .` after metadata sync. A schema violation fails the release loudly instead of shipping a silently-stripped manifest. Follow-up to make the sync schema-aware if the schema evolves rapidly. |
...
| Inline `node -e` blocks in `release.sh` drift from the schema and become hard to maintain. | Extract to a standalone `scripts/release-sync-manifests.mjs` file if the logic grows further. Tracked as a follow-up. |
